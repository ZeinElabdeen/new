...
import 'package:haat/src/Screens/HomePages/Apis/Models/AppModels/chat_msg_model.dart';
...
class ChatBloc extends Bloc<AppEvent, AppState> {
  ...
  Stream<AppState> mapEventToState(AppEvent event) async* {
    if (event is UploadPhoto) {
      print("sender id------------- ${_secondUserID.value}");
    ...
    if (event is Init) {
      ...
      OldMessagesModel _old = await _api.getMessages(chatID: _chatID.value);
      if (_old.code == 200) {
